...
extension String {
    ...
    func sliderAttributeString(intFont: UIFont, decimalFont: UIFont, customDecimalSeparator: String? = nil) -> NSAttributedString {
        ...
        if #available(iOS 8.2, *) {
            integer.addAttributes([NSAttributedStringKey.font: UIFont.systemFont(ofSize: 42, weight: .regular)], range: NSMakeRange(0, integer.length))
            decimal.addAttributes([NSAttributedStringKey.font: UIFont.systemFont(ofSize: 42, weight: .thin)], range: NSMakeRange(0, decimal.length))
        } else {
